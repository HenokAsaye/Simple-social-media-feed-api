...
export const likePost = async(req,res)=>{
    ...
    try{
        ...
        const likeExists = await like.findOne({postId:postId,user:req.user._id});
...
export const unlikePost = async(req,res)=>{
    ...
    try{
        ...
        const unlike = await like.deleteOne({postId:postId})
        await unlike.save();
        findPost.likeCount = (Post.likeCount)+1
        await findPost.save();
...
export const LikeComment  = async(req,res)=>{
    ...
    try{
        ...
        const likeExists = await like.findOne({commentId:commentId,user:req.user._id})
        if(likeExists){
            return res.status(400).json({message:"you have already liked this comment!",error:error.message})
        }
        const newLike = {
            comment:commentId,
            user:req.user._id
...
export const UnlikeComment = async(req,res)=>{
    ...
    try{
        ...
        if(!liked){
            return res.status(400).json({message:"ypu haven't like the comment before"})
        }
        const unlike = await like.deleteOne(commentId);
        await unlike.save()

        findComment.likeCount = (Comment.likeCount)-1
        await findComment.save();
        res.status(200).json({messgae:"Comment unliked successfully",Comment})
